...
// ORG__Close: the vtable contents aren't final until every method of the
...
// Cached target info — set once on first ORG__Open and reused for every
// emitted module. Triple/layout get stamped onto each module so the
// generated .ll is self-describing; HostTM is kept alive so ORG__Close can
 ...
 * ORG_SetTargetTriple() before the first ORG__Open(). When NULL we use
...
// Driver flags controlling what ORG__Close writes out. Defaults match the
...
// (reset at ORG__Enter and after ORG__Header's import-init chain). If still
// zero at ORG__Return / ORG__Close and the procedure returns nothing, the
...
// our module's prefix and pick up internal/external linkage in ORG__Enter.
...
static LLVMValueRef LookupProc(ORB_Object *proc) {
    ...
    // by ORG__Header for an imported module's init function).
...
// to finalize_tds() (ORG__Close): the vtable isn't complete until all of
...
// Fill in the deferred TD bodies + initializers. Called from ORG__Close,
...
static LLVMValueRef LoadItem(ORG_Item *x) {
    if (x->mode == ORB_Const) {
        ...
        // x->backend (set by ORG__MakeItem via LookupProc). The generic
...
void ORG__MakeConstItem(ORG_Item *x, ORB_Type *typ, LONGINT val) {
    x->mode = ORB_Const;
    x->type = typ;
    x->a = val; x->b = 0; x->r = 0;
    x->rdo = FALSE;
...
void ORG__MakeRealItem(ORG_Item *x, REAL val) {
...
void ORG__MakeStringItem(ORG_Item *x, LONGINT len) {
    x->mode = ORB_Const;
    x->type = strType;
    // x->a holds the first byte so `s := "x"` (one-char string → CHAR) folds
    // into a constant store via ORG__StrToChar without a load.
...
LONGINT ORG__StrOffset(void) { return 0; }
void ORG__PutByte(int b) { (void)b; }
...
void ORG__MakeItem(ORG_Item *x, ORB_Object *y, LONGINT curlev) {
    (void)curlev;
    x->mode = y->class;
    x->type = y->type;
    x->a = y->val;
    ...
    if (y->class == ORB_Var) {
        ...
        // lev > 0: a procedure local (alloca set up in ORG__Enter).
        if (y->lev <= 0) {
...
void ORG__Field(ORG_Item *x, ORB_Object *y) {
    LLVMTypeRef rec_ty = LlvmType(x->type);
    if (!x->backend) {
        ORS_Mark("internal: ORG__Field with no backend");
        return;
    }
...
void ORG__Index(ORG_Item *x, ORG_Item *y) {
    if (!x->backend) {
        ORS_Mark("internal: ORG__Index with no backend");
        return;
    }
...
//
// The receiver value is stashed in x->backend2; ORG__PrepCall pushes it as
// the hidden first argument when the call frame opens.
void ORG__MethodItem(ORG_Item *x, ORB_Object *m, BOOLEAN direct) {
    ...
    if (direct) {
        fn = LookupProc(m);
        x->mode = ORB_Const;   // like a named-procedure item (ORG__MakeItem)
    } else {
...
// with the instance stashed as the hidden receiver (ORG__PrepCall pushes
// it). The init body returns the instance at +1 (ORG__Return's pointer
// convention), which the caller consumes by store or parameter pass.
void ORG__InitItem(ORG_Item *x, ORB_Object *m, ORB_Type *rec) {
...
void ORG__PtrToIface(ORG_Item *x, ORB_Type *ift) {
...
void ORG__IfaceMethodItem(ORG_Item *x, ORB_Object *m) {
    ...
    x->mode = Reg;
    x->backend = fn;
    x->backend2 = data;   // hidden receiver for ORG__PrepCall
    x->a = 0;
    x->b = 0;
    x->r = 0;
    x->rdo = FALSE;
...
void ORG__StoreIface(ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
...
void ORG__IfaceRelation(INTEGER op, ORG_Item *x, ORG_Item *y) {
...
void ORG__BuildTD(ORB_Type *T, LONGINT *dc) { (void)T; (void)dc; }
void ORG__TypeTest(ORG_Item *x, ORB_Type *T, BOOLEAN varpar, BOOLEAN isguard) {
    (void)varpar;
...
void ORG__And2(ORG_Item *x, ORG_Item *y) {
...
void ORG__Or2(ORG_Item *x, ORG_Item *y) {
    LLVMBasicBlockRef skip = (LLVMBasicBlockRef)x->backend2;
    if (!skip) {
        LLVMValueRef a = LoadItem(x), b = LoadItem(y);
        x->mode = Cond;
...
void ORG__AddOp(LONGINT op, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    coerce_pair(x, y, &a, &b);
    LLVMValueRef r;
    if (op == ORS_plus) {
...
void ORG__MulOp(ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    coerce_pair(x, y, &a, &b);
    x->mode = Reg;
...
void ORG__DivOp(LONGINT op, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    coerce_pair(x, y, &a, &b);
    x->mode = Reg;
...
void ORG__RealOp(INTEGER op, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    LLVMValueRef r;
...
// --- Sets — placeholder ---
void ORG__Singleton(ORG_Item *x) {
...
void ORG__Set(ORG_Item *x, ORG_Item *y) {
...
void ORG__In(ORG_Item *x, ORG_Item *y) {
...
void ORG__SetOp(LONGINT op, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    LLVMValueRef r;
...
void ORG__IntRelation(INTEGER op, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    coerce_pair(x, y, &a, &b);
...
void ORG__RealRelation(INTEGER op, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
...
void ORG__StringRelation(INTEGER op, ORG_Item *x, ORG_Item *y) {
...
// --- Assignment ---
void ORG__StrToChar(ORG_Item *x) {
    x->type = charType;
    x->orig_type = charType;
}

void ORG__Store(ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
...
void ORG__StoreStruct(ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
...
void ORG__CopyString(ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
...
// --- Parameter passing ---
void ORG__OpenArrayParam(ORG_Item *x) {
...
void ORG__VarParam(ORG_Item *x, ORB_Type *ftype) {
...
void ORG__ValueParam(ORG_Item *x, ORB_Type *ftype) {
    if (CallTop < 0 || CallArgC[CallTop] >= MAX_CALL_ARGS) return;
    LLVMValueRef v = LoadItem(x);
...
void ORG__StringParam(ORG_Item *x) {
    if (CallTop < 0 || CallArgC[CallTop] + 1 >= MAX_CALL_ARGS) return;
...
//   BJump(L0)
//   FixLink(L1)
void ORG__For0(ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
    LLVMValueRef rhs = LoadItem(y);
    LLVMTypeRef dst_ty = LlvmType(x->type);
    LLVMTypeRef src_ty = LLVMTypeOf(rhs);
...
void ORG__For1(ORG_Item *x, ORG_Item *y, ORG_Item *z, ORG_Item *w, LONGINT *L) {
    (void)y;
...
void ORG__For2(ORG_Item *x, ORG_Item *y, ORG_Item *w) {
    (void)y;
    EmittedStmts++;
    LLVMValueRef xv = LoadItem(x);
...
void ORG__CaseLabel(ORG_Item *x, LONGINT val, LONGINT *chain) {
    LLVMValueRef xv = LoadItem(x);
...
// CaseRange: lo <= x <= hi.
void ORG__CaseRange(ORG_Item *x, LONGINT lo, LONGINT hi, LONGINT *chain) {
    LLVMValueRef xv = LoadItem(x);
    LLVMTypeRef xt = LLVMTypeOf(xv);
...
void ORG__CBJump(ORG_Item *x, LONGINT L) {
...
// --- Procedure calls ---
void ORG__PrepCall(ORG_Item *x, LONGINT *r) {
    ...
    // Type-bound call: the receiver stashed by ORG__MethodItem becomes the
...
void ORG__Call(ORG_Item *x, LONGINT r) {
    EmittedStmts++;
    LLVMValueRef fn;
    ...
    // after returning from ORG__Call.
    if (has_result) {
        x->mode = Reg;
        x->backend = result;
...
void ORG__Enter(ORB_Object *proc, ORB_Object *params, LONGINT parblksize, LONGINT locblksize, BOOLEAN int_proc) {
...
void ORG__Return(ORB_Object *proc, ORG_Item *x) {
    ...
    CurFn = ModInit;   // resume module-init context (set up later in ORG__Header)
}

// --- Built-ins ---
void ORG__Increment(LONGINT upordown, ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
...
void ORG__Include(LONGINT inorex, ORG_Item *x, ORG_Item *y) {
    EmittedStmts++;
    LLVMValueRef addr = LValueItem(x);
...
void ORG__Assert(ORG_Item *x) { EmittedStmts++; (void)x; }

void ORG__New(ORG_Item *x) {
    EmittedStmts++;
    if (!x->type || x->type->form != ORB_Pointer) {
    ...
    // Same pattern as ORG__Store on a pointer: retain new, release old, store.
...
void ORG__Pack(ORG_Item *x, ORG_Item *y) { EmittedStmts++; (void)x; (void)y; }
void ORG__Unpk(ORG_Item *x, ORG_Item *y) { EmittedStmts++; (void)x; (void)y; }
...
void ORG__Get(ORG_Item *addr, ORG_Item *var) {
    EmittedStmts++;
...
void ORG__Put(ORG_Item *addr, ORG_Item *val) {
    EmittedStmts++;
    LLVMValueRef p = addr_to_ptr(addr);
...
void ORG__Copy(ORG_Item *src, ORG_Item *dst, ORG_Item *n) {
    EmittedStmts++;
...
void ORG__Shift(LONGINT fct, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    LLVMTypeRef at = LLVMTypeOf(a);
...
void ORG__Bitwise(LONGINT fct, ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
    coerce_pair(x, y, &a, &b);
    x->mode = Reg;
...
void ORG__UML(ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
...
void ORG__Bit(ORG_Item *x, ORG_Item *y) {
    LLVMValueRef a = LoadItem(x), b = LoadItem(y);
...
void ORG__SetDataSize(LONGINT dc) {
    ORG_varsize = dc;
}

void ORG__Header(void) {
    ...
    //     br label %init_done    ; emitted by ORG__Close
    //   init_done:
    //     ret void               ; emitted by ORG__Close
    //
...
void ORG__Close(const char *modid, INTEGER modid_len, LONGINT key, LONGINT nofent) {
    ...
    if (!ModInit) {
        // Defensive: ORG__Header wasn't reached (parse error path).
        LLVMTypeRef ft = LLVMFunctionType(Ty_void, NULL, 0, 0);
        char name[200];
