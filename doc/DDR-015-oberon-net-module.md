...
**Resolved — the stream core carries a `(start, len)` window.** `Reader.Read(VAR buf: ARRAY OF BYTE; start, len: INTEGER): INTEGER` fills `buf[start ..< start+len]`; `Writer.Write(buf: ARRAY OF BYTE; start, len: INTEGER): INTEGER` writes `len` bytes from `buf[start]`. This amends DDR-008's `Read(buf)`/`Write(buf)` (2026-08-04, decided with the author).

Rationale: Oberon-07 has **no array slices and no safe pointer-into-a-buffer** — you cannot pass "`buf` from offset 256" as an open array without dropping to `SYSTEM.ADR`, which has no place in the portable stream surface. The window is therefore how a caller streams a chunk of a larger buffer in either direction without copying: the pointer arithmetic is done once, safely, in the backing C (`recv(fd, buf+start, m)` / `send(fd, buf+start, m)`), and `start`/`len` are clamped to `buf`'s bounds there. It is strictly more general than a lone `n` (it expresses "the middle chunk", not just "the first n"), and it keeps the interface a plain method — no `Buffer`-struct machinery. Whole-buffer I/O is just `Read(buf, 0, LEN(buf))` / `Write(buf, 0, LEN(buf))`. `tests/NetTest.Mod` exercises a `start ≠ 0` write.
