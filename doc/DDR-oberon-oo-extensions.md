...
- **Error model.** *Resolved 2026-07-16 by DDR-009 (`doc/DDR-009-oberon-error-model.md`): errors travel in a trailing **`VAR` error out-parameter** (the Wirth `VAR res` idiom, upgraded); results stay in RETURN position; NIL/0 = success; callee assigns on every path.* Exceptions, Go-style tuple returns, error-as-result, and sum types are recorded as rejected alternatives. *Residual open item (DDR-009 §4):* ratify the error **type** — recommendation is a pointer to an extensible record in a core `Errors` module — before library implementation.
...
5. **Error model** — mechanism decided (DDR-009: VAR error out-parameter); ratify the error type (DDR-009 §4) and migrate the runtime's existing `VAR res: INTEGER` procedures before any library surface commits to a convention.
